...
#
# Key fix (dataset-agnostic):
#   All calls to build_model() and evaluate_cheap() now receive the correct
#   input_size derived from cfg.get_input_size(), making the loop work for
#   CIFAR-10 (32×32), CIFAR-100 (32×32), TINY-IMAGENET (64×64), and
#   IMAGENET (224×224) without any hardcoded tensor shapes.
# =============================================================================
...
def _train_one_child_gpu(pc, cfg, train_epochs, train_lr,
    ...
    Uses cfg.get_input_size() for all model construction.
    """
    import torch
    from objectives.cheap import clean_state_dict

    input_size = cfg.get_input_size()   # ← dataset-aware, no hardcoding
    ...
    try:
        ...
        # ------------------------------------------------------------------
        # 7. Move model back to CPU to free GPU memory
        # ------------------------------------------------------------------
        child_model.cpu()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
...
# =============================================================================
# CPU path — parallel training via ProcessPoolExecutor
# =============================================================================
...
def _worker_train_child(idx, pickled_payload, cfg, train_epochs, train_lr):
    ...
    input_size = cfg.get_input_size()  # ← dataset-aware
    ...
    try:
        ...
        try:
            child.evaluate_cheap(
                objective_keys=cfg.CHEAP_OBJECTIVES,
                input_size=input_size,          # ← dataset-aware
            )
        except Exception:
            child.f_cheap = {k: 0 for k in cfg.CHEAP_OBJECTIVES}
...
def _parallel_train_cpu(payloads, cfg, train_epochs, train_lr, desc):
    """Train children in parallel on CPU cores (no GPU available)."""
    import torch
        ...
        for fut in iterator:
            ...
            try:
                ...
                if sd_path and os.path.exists(sd_path):
                    model = ind.build_model(input_shape=input_size)  # ← fixed
...
def run_lemonade(init_graphs, cfg, train_loader, val_loader, device, run_dir):
    ...
    cfg          : NASConfig
    train_loader : DataLoader (main process loaders — GPU path recreates its own)
    ...
    for gen in range(1, cfg.GENERATIONS + 1):
        ...
        try:
            # ----------------------------------------------------------
            # STEP 1: KDE fit on current population
            # ----------------------------------------------------------
            ...
            for p in parents:
                try:
                    ...
                    try:
                        ...
                        parent_model = p.build_model(input_shape=input_size)      # ← fixed
                        child_model  = child.build_model(input_shape=input_size)  # ← fixed
