...
_DATASET_META = {
    "CIFAR-10":  (10,   128),
    "CIFAR-100": (100,  128),
    "IMAGENET":  (1000,  32),   # smaller batch for large images
}

NUM_CLASSES = _DATASET_META[CFG.TARGET_DATASET][0]
# Override batch size if dataset has a tighter default
if CFG.BATCH_SIZE == 128 and CFG.TARGET_DATASET == "IMAGENET":
    CFG.BATCH_SIZE = 32
...
def create_seed_population(num_seeds: int) -> list:
    ...
    for _ in range(num_seeds - 1):
        for _ in range(15):        # up to 15 attempts to produce a valid variant
            ...
            # Fallback: use a copy of base (better than crashing)
...
def main():
    ...
    for i, ind in enumerate(final_population):
        ...
        logger.info(
            "Final model %d | id=%s params=%s flops=%s "
            "val_err=%s test_err=%s",
            i, ind.id,
